...
public class ThreadServer {
			...
			public void run(){
				while(true){
					...
					if(re.getString(re.getClr()).equals("connect")){
						//there need an login-in function which can receive account and password from the client
						a.add(re.getClr());
	...
	public void t1_start(DatagramPacket p){
		t1 = new Thread(){
			public void run(){
				...
				s1.sendJ("127.0.0.1", Integer.parseInt(s1.getString(p)), "request");
				while(true){
	...
	public void t2_start(DatagramPacket p){
		t2 = new Thread(){
			public void run(){
				...
				s2.sendJ(s2.getIP(p), s2.getPort(p), "request");
				while(true){
					//every while loop should have limit time of waiting , if cannot rece the info from client ,kill it
					s2.receiveJS();
	...
	public void t3_start(DatagramPacket p){
		t2 = new Thread(){
			public void run(){
				...
				s3.sendJ(s3.getIP(p), s3.getPort(p), "request");
				while(true){
					//every while loop should have limit time of waiting , if cannot rece the info from client ,kill it
					s3.receiveJS();
	...
	public static void main(String[] args){
		ThreadServer a = new ThreadServer(5050,5051,5052,5053);
		a.process();
	}
}